...
<head>
    ...
    <%
        ...
        if (session.getAttribute("groupid").toString().isEmpty()) {
    %>
    <script>
        alert("<%=session.getAttribute("groupid").toString()%>")
    ...
    <%
        ...
        try {
            dtoList = dao.searchAlluserid(session.getAttribute("userid").toString(), session.getAttribute("groupid").toString());
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function (info) {
                    ...
                    else if (info.event.extendedProps.wp !== "<%=session.getAttribute("username")%>"){
                        $("#calendar_title_readonly").prop("value", info.event.title);
                        let data = info.event.startStr.split(":")
                        let data1 = info.event.endStr.split(":")

                        //날자 포맷 다시 보기 트러블 슈팅 리포트 작성
                        $("#calendar_writer_readonly").prop("value",info.event.extendedProps.wp);
                        $("#calendar_start_date_readonly").prop("value", data[0] + ":" + data[1]);
                        $("#calendar_end_date_readonly").prop("value", data1[0] + ":" + data1[1])
                        $("#calendar_description_readonly").val(info.event.extendedProps.content);
                        $("#scheduledid_readonly").prop("value", info.event.extendedProps.scheduledid);
                        console.log(info.event.extendedProps.scheduledid)
                        $("#calendarModal_readonly").modal("show");
                    }
                    else {
                ...
                dateClick: function (info) {
                    ...
                    $("#form-action").prop("action", "../controller/insertschedulegroup.jsp");
                ...
                events: [
                    <% for (calendarDto dto: dtoList){%>
                    {
                        ...
                        wp:"<%=dao1.searchByIdForUsername(dto.getUserid())%>"
                    },
                    <% } %>
                ]
            });
            ...
            <form action="../controller/insertschedule.jsp" method="post" id="form-action">
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="datetime-local" class="form-control" id="calendar_start_date" name="startdt"
                               value="">
...
<!--modal v2-->
<div class="modal fade" id="calendarModal_readonly" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
     aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel_readonly">일정 확인</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                </button>
            </div>
            <form action="../controller/insertschedule.jsp" method="post" id="form-action_readonly">
                <div class="modal-body">
                    <div class="form-group">
                        <input hidden="hidden" type="text" value="" name="scheduledid" id="scheduledid_readonly">
                        <label class="col-form-label">일정 제목</label>
                        <input type="text" class="form-control" id="calendar_title_readonly" name="title" value="">
                        <label class="col-form-label">작성자</label>
                        <input type="text" class="form-control" id="calendar_writer_readonly" name="writer" value="">
                        <label class="col-form-label">시작 날짜</label>
                        <input type="datetime-local" class="form-control" id="calendar_start_date_readonly" name="startdt"
                               value="">
                        <label class="col-form-label">종료 날짜</label>
                        <input type="datetime-local" class="form-control" id="calendar_end_date_readonly" name="enddt" value="">
                        <label class="col-form-label">일정 내용</label>
                        <textarea class="form-control" aria-label="일정내용" name="description"
                                  id="calendar_description_readonly"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"
                            id="sprintSettingModalClose_readonly">취소
                    </button>
                </div>
            </form>
        </div>
